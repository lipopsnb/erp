                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <?php else: ?>
                        <?php foreach ($deliveries as $dv):
                            ...
                            $st = ['draft'=>['secondary','Nh��p'],'confirmed'=>['primary','Xác nhận'],'invoiced'=>['success','Xuất HĐ']];
...
document.getElementById('modalDelivery').addEventListener('show.bs.modal', () => {
    ...
    rowIdx = 0;
    document.getElementById('itemBody').insertAdjacentHTML('beforeend', makeRow(rowIdx++, 'itemBody'));
...
document.getElementById('btnAddEditRow').addEventListener('click', () => {
    document.getElementById('editItemBody').insertAdjacentHTML('beforeend', makeRow(editRowIdx++, 'editItemBody'));
});
...
// ── Mở modal sửa: load items từ server ───────────────────────
document.querySelectorAll('.btn-edit-delivery').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        for (let opt of sel.options) {
            if (opt.value == d.customerId) { opt.selected = true; break; }
        }
        ...
        .then(res => {
            ...
            if (res.ok && res.items.length > 0) {
                res.items.forEach(item => {
                    body.insertAdjacentHTML('beforeend', makeRow(editRowIdx++, 'editItemBody'));
