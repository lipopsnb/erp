...
class PayrollEngine
{
    ...
    // ─────────────────────────────────────────────────────────────────────
    // PUBLIC: Tính ngày công chuẩn (loại Chủ nhật)
    // ─────────────────────────────────────────────────────────────────────
    ...
    public function calculate(int $periodId, int $userId): array
    {
        ...
        $holidayPaidDays    = $this->getHolidayWorkDays($period['period_from'], $period['period_to']);
        ...
        // ── Trợ cấp thực nhận ────────────────────────────────────────
        // meal_received = (mealAllow * tỷ lệ ngày làm) + ăn ca OT
        ...
        foreach ($salary['all_components'] as $comp) {
            ...
            if (in_array($code, [
                ...
                'transport', 'performance', 'attendance_bonus'
            ])) continue;
        ...
        // ── BHXH: chỉ tính khi has_social_insurance = 1 ─────────────
        ...
        $grossForTax = $basicReceived
                     + $mealReceived        // đã bao gồm ăn ca OT
                     + $clothesReceived
                     + $phoneReceived
                     + $transportReceived
                     + $performReceived
        ...
        return [
            ...
            'meal_received'              => $mealReceived,   // ← đã gộp ăn ca OT
            'ot_meal_days'               => $otMealDays,     // ← số ngày OT ≥ 3h
            'ot_meal_bonus'              => $otMealBonus,    // ← tiền ăn ca OT
    ...
    // ─────────────────────────────────────────────────────────────────────
    // PUBLIC: Lương/ngày cho 1 user — dùng cho KPI result.php
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
// PRIVATE: Đếm số ngày OT có tổng giờ >= 3h (được hưởng ăn ca OT)
//          CHỈ tính ngày thường — loại Chủ nhật (DAYOFWEEK=1) + ngày lễ
// ─────────────────────────────────────────────────────────────────────
private function getOTMealDays(int $userId, string $from, string $to): int
{
    try {
        // Lấy danh sách ngày lễ trong kỳ
        ...
        // Gom OT theo ngày, chỉ lấy ngày thường (không phải CN, không phải ngày lễ)
        // rồi đếm những ngày có tổng >= 3h
        $stmt = $this->pdo->prepare("
            ...
            FROM (
                ...
                WHERE user_id = ?
                  ...
                  AND DAYOFWEEK(ot_date) != 1          -- loại Chủ nhật
                  AND ot_date NOT IN ($holidayList)    -- loại ngày lễ
                GROUP BY ot_date
                HAVING total_hours >= ?
            ) AS daily_ot
        ");
    ...
    // ─────────────────────────────────────────────────────────────────────
    // PRIVATE: Tính ngày phép năm theo tháng làm việc thực tế
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
    private function getHolidayWorkDays(string $from, string $to): float
    {
        $stmt = $this->pdo->prepare(
            "SELECT holiday_date FROM holidays WHERE holiday_date BETWEEN ? AND ?"
        );
        ...
        foreach ($holidays as $hDate) {
            if ((int)(new DateTime($hDate))->format('N') !== 7) $count++;
        }
        return (float)$count;
    }
    ...
    // ─────────────────────────────────────────────────────────────────────
    // PRIVATE: Ngày phép annual đã dùng trong năm
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
    // PRIVATE: Thuế TNCN 7 bậc lũy tiến
    // ─────────────────────────────────────────────────────────────────────
